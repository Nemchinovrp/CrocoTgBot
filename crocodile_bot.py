...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        "🔹 Для остановки игры - /stop_croco\n"
        "🔹 Когда слово угадано, просто напишите его в чат!"
    )

async def stop_croco(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Останавливает текущую игру"""
    chat_id = update.effective_chat.id
    
    if chat_id not in game_states:
        await update.message.reply_text("ℹ️ В данный момент игра не активна.")
        return
    
    game_state = game_states[chat_id]
    
    if game_state.current_leader is None:
        await update.message.reply_text("ℹ️ В данный момент игра не активна.")
        return
    ...
    await update.message.reply_text("🛑 Игра была остановлена. Для новой игры используйте /play")
...
async def play(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if game_state.current_leader is not None:
        await update.message.reply_text(
            "❗ Игра уже идет! Дождитесь окончания или используйте /stop_croco для остановки."
        )
        return
...
def main() -> None:
    """Запуск бота"""
    application = Application.builder().token("7734707595:AAExtqroKgYL7rtCKAZPb8sBhFPaxRoAB9A").build()
    
    application.add_handler(CommandHandler("start", start))
    application.add_handler(CommandHandler("play", play))
    application.add_handler(CommandHandler("stop_croco", stop_croco))
